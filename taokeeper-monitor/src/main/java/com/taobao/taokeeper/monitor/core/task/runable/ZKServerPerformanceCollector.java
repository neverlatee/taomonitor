...
public class ZKServerPerformanceCollector implements Runnable
{
	...
	public void run()
	{
		try
		{
			HostPerformanceEntity hostPerformanceEntity = SSHUtil.getHostPerformance(ip, SystemConstant.portOfSSH,
					userNameOfSSH, passwordOfSSH, SystemConstant.identityOfSSH);
			if (hostPerformanceEntity == null)
			{
				return;
			}
		...
		{
			LOG.error("程序出错: " + exception.getMessage(), exception);
	...
	public void sendAlarm(AlarmSettings alarmSettings, HostPerformanceEntity hostPerformanceEntity, String clusterName)
	{
		...
		{ // Cpu usage alarm
			...
			if (!StringUtil.isBlank(cpuUsage) && cpuUsage.endsWith("%"))
			{
				...
				if (0 < difference)
				{
					LOG.warn("ZK Server " + hostPerformanceEntity.getIp() + " cpu Usage too high：" + cpuUsage + "-"
							+ maxCpuUsage + "=" + difference);
					// Alarm
		...
		{ // Memory usage alarm
			...
			if (!StringUtil.isBlank(memoryUsage) && memoryUsage.endsWith("%"))
			{
				...
				if (0 < difference)
				{
					LOG.warn("ZK Server " + hostPerformanceEntity.getIp() + " memory usage too high: " + memoryUsage
							+ "-" + maxMemoryUsage + "=" + difference);
					// Alarm
					if (GlobalInstance.needAlarm.get())
					{
		...
		{ // Load usage alarm
			...
			if (!StringUtil.isBlank(load))
			{
				...
				if (0 < difference)
				{
					LOG.warn("ZK Server " + hostPerformanceEntity.getIp() + " load usage too high: " + load + "-"
							+ maxLoad + "=" + difference);
					if (GlobalInstance.needAlarm.get())
					{
		...
		try
		{
			if (!StringUtil.isBlank(dataDir) || !StringUtil.isBlank(dataLogDir))
			{ // 需要进行
				// disk容量
				// 报警
				...
				if (!StringUtil.isBlank(maxDiskUsage))
				{
					...
					if (null != diskUsageMap)
					{
						for (String mountedOn : diskUsageMap.keySet())
						{
							...
							{
								int diskUsage = Integer.parseInt(StringUtil.trimToEmpty(diskUsageMap.get(mountedOn))
										.replace(PERCENT, EMPTY_STRING));
								if (diskUsage > Integer.parseInt(maxDiskUsage))
								{
									LOG.warn("ZK Server " + hostPerformanceEntity.getIp() + " disk usage too high, "
