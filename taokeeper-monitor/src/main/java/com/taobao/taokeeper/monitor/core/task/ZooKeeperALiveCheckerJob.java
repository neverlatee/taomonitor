 ...
 * 通过对/ZOOKEEPER.MONITOR.ALIVE.CHECK这个持久节点的读写来监控zk节点的存活性
		...
		while (true)
		{
			...
			try
			{
				...
				try
				{
					...
					{
						for (final ZooKeeperCluster zookeeperCluster : zooKeeperClusterSet)
						{ // 对每个cluster处理
							...
							if (null != zookeeperCluster && null != zookeeperCluster.getServerList())
							{
								...
								{
									// 一定要进行释放。
									GlobalInstance.removeFromAllCheckingCluster(zookeeperCluster.getClusterId()
											+ EMPTY_STRING);
									continue;
								}
								ThreadPoolManager.addJobToZooKeeperNodeAliveCheckExecutor(new ZKServerAliveCheck(
				...
				{
					LOG.error("Exception when check zk server alive, Error: ", e);
				}
				// 每1分钟收集一次检测
