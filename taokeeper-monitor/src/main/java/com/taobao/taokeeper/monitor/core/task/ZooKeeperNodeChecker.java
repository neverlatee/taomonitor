...
public class ZooKeeperNodeChecker extends TimerTask
{
	...
	private void checkNodePath(final ZooKeeperCluster zookeeperCluster)
	{
		...
		LOG.info("开始检查ZK集群，name: " + zookeeperCluster.getClusterName() + ", serverList: "
				+ zookeeperCluster.getServerList() + " " + nodePathCheckRule);
		//		基本逻辑已经ok，明天做一些测试就好了~
			...
			for (String path : pathCanNotBeExist.keySet())
			{
				...
				for (String pathConfig : listConfig)
				{
					...
					if (listReal.contains(pathConfig))
					{
						...
						{
							SendMessageUtil.sendYxMessage(phoneList, "ZooKeeper Node的path检查结果,cluster: "
									+ zookeeperCluster.getClusterName() + "| " + path + " 下存在多余的node：" + pathConfig);
						}
					}
				}
			}
