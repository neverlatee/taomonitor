...
<h1>
	ZooKeeper集群状态<font size="2">
		更新时间：${timeOfUpdateZooKeeperStatusSet }</font><font size="2"> <a
		href="zooKeeper.do?method=zooKeeperRegisterPAGE">加入监控</a></font>
</h1>
...
<select id="clusterSelector"
	...
	<c:forEach var="zooKeeperCluster" items="${zooKeeperClusterMap}">
		<c:choose>
			<c:when test="${ zooKeeperCluster.key eq clusterId }">
				<option value="${zooKeeperCluster.key}" selected>${zooKeeperCluster.value.clusterName}</option>
			</c:when>
			<c:otherwise>
				<option
					value="zooKeeperStatus.do?method=showZooKeeperStatusPAGE&clusterId=${zooKeeperCluster.key}">${zooKeeperCluster.value.clusterName}</option>
			</c:otherwise>
		</c:choose>
	</c:forEach>
</select>
...
<div align="left" class="mytable" id="tab">
	<table border="0" cellspacing="0" cellpadding="0">
		...
		<c:forEach var="zooKeeperStatus" items="${zooKeeperStatusMap}">
			<tr>
				...
				<td>&nbsp;<img style="cursor: pointer;" src="img/seeDetail.png"
					onclick="openDialog('${zooKeeperStatus.value.statContent}')" /></td>

				<c:choose>
					<c:when test="${ zooKeeperStatus.value.statusType eq 1 }">
						<td style="background-color: green;">OK</td>
					</c:when>
					<c:when test="${ zooKeeperStatus.value.statusType eq 0 }">
						<td style="background-color: yellow;">Checking</td>
					</c:when>
					<c:when test="${ zooKeeperStatus.value.statusType eq -1 }">
						<td style="background-color: #D1F3D6;">Initializing</td>
					</c:when>
					<c:otherwise>
	...
	<br />
	<br /> <b>提示</b>:<br>
	<br> 1. 节点自检 是指对集群中每个IP所在ZK节点上的PATH: <b>/YINSHI.MONITOR.ALIVE.CHECK</b>
	定期进行三次如下流程 : <br /> <b>节点连接</b> - <b>数据发布</b> - <b>修改通知</b> - <b>获取数据</b>
	- <b>数据对比</b>, 三次流程均成功视为该节点处于正常状态。<br>
	<br> 2. 角色分类：<b>L</b>: Leader, <b>F</b>: Follower, <b>O</b>:
	Observer, <b>S</b>: Standalone
