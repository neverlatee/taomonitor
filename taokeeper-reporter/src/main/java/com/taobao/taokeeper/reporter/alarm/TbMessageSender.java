...
public class TbMessageSender implements MessageSender
{
	...
	public void run()
	{
		...
		{
			LOG.info("[TaoKeeper]No need to send message: messages.length: " + messages + ", IP_OF_MESSAGE_SEND="
					+ SystemConstant.IP_OF_MESSAGE_SEND);
			return;
		}
		...
		for (Message message : this.messages)
		{
			try
			{
				this.sendMessage(StringUtil.trimToEmpty(message.getTargetAddresses()),
						StringUtil.trimToEmpty(message.getSubject()), StringUtil.trimToEmpty(message.getContent()),
