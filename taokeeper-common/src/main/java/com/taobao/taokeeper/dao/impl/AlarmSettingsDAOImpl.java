...
public class AlarmSettingsDAOImpl implements AlarmSettingsDAO
{
	...
	public AlarmSettings getAlarmSettingsByCulsterId(int clusterId) throws DaoException
	{
		...
		try
		{
			...
			if (null != rs && rs.next())
			{
				...
				String needAlarm = rs.getString("need_alarm");
				...
				alarmSettings.setNeedAlarm(needAlarm);
		...
		{
			throw new DaoException("Error when query AlarmSettings by cluster_id: " + clusterId + ", Error: "
					+ e.getMessage(), e);
		}
		finally
		{
	...
	public boolean updateAlarmSettingsByClusterId(AlarmSettings alarmSettings) throws DaoException
	{
		...
		try
		{
			String updateSql = StringUtil.replaceSequenced(SQL_UPDATE_ALARM_SETTINGS_BY_ID, //
					...
					alarmSettings.getNeedAlarm(), alarmSettings.getClusterId() + EMPTY_STRING);
		...
		{
			throw new DaoException("Error when update AlarmSettings by cluster_id: " + alarmSettings + ", Error: "
					+ e.getMessage(), e);
		}
	}
	...
	public boolean addAlarmSettings(AlarmSettings alarmSettings) throws DaoException
	{
		...
		try
		{
			String insertSql = StringUtil.replaceSequenced(SQL_ADD_ALARM_SETTINGS, alarmSettings.getClusterId()
					+ EMPTY_STRING, alarmSettings.getMaxDelayOfCheck(), alarmSettings.getMaxCpuUsage(),
					alarmSettings.getMaxMemoryUsage(), alarmSettings.getMaxLoad(), alarmSettings.getWangwangList(),
					alarmSettings.getPhoneList(), alarmSettings.getEmailList(), alarmSettings.getMaxConnectionPerIp(),
					alarmSettings.getMaxWatchPerIp(), alarmSettings.getDataDir(), alarmSettings.getDataLogDir(),
					alarmSettings.getMaxDiskUsage(), alarmSettings.getNeedAlarm());
